...
export default function Payroll() {
  ...
  const load = async () => { setLoading(true); setList(await listPayroll({ month, year })); setLoading(false); };
  ...
  const generate = async (e) => {
    ...
    setBusy(true);
    try {
      await generatePayroll({ employee_id: genFor, month, year, incentive: genIncentive, bonus: genBonus, overtime: genOvertime, extra_deductions: genExtraDed, notes: genNotes });
    ...
    <div className="sk-page space-y-5" data-testid="payroll-page">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        ...
        <div className="flex gap-2">
          <select value={month} onChange={e => setMonth(Number(e.target.value))} className="sk-input w-auto">{MONTHS.slice(1).map((m, i) => <option key={i + 1} value={i + 1}>{m}</option>)}</select>
          <select value={year} onChange={e => setYear(Number(e.target.value))} className="sk-input w-auto">{[year - 1, year, year + 1].map(y => <option key={y} value={y}>{y}</option>)}</select>
        </div>
      </div>
      {isAdmin && (
        <form onSubmit={generate} className="sk-card p-5">
          <div className="flex items-center gap-2 mb-3"><Sparkles className="w-4 h-4 text-[#FFA94D]" /><div className="font-heading font-bold">Generate / Update Payroll for {MONTHS[month]} {year}</div></div>
          <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
            <select required value={genFor || ""} onChange={e => setGenFor(e.target.value)} className="sk-input md:col-span-2">
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button disabled={busy} className="sk-btn-primary">{busy && <Loader2 className="w-4 h-4 animate-spin" />} Generate</button>
          </div>
      ...
      <div className="sk-card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2"><Wallet className="w-4 h-4 text-[#4DA3FF]" /><div className="font-heading font-bold">{MONTHS[month]} {year}</div></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-xs uppercase tracking-wider text-slate-500 border-b">
              <th className="py-2.5 px-5">Employee</th><th className="py-2.5 pr-4">Days (P/H/A)</th><th className="py-2.5 pr-4 text-right">Base</th>
              <th className="py-2.5 pr-4 text-right">+ Incentive</th><th className="py-2.5 pr-4 text-right">+ Bonus</th>
              <th className="py-2.5 pr-4 text-right">- Deductions</th><th className="py-2.5 pr-4 text-right">Net</th>
            ...
            <tbody>
              {loading ? <tr><td colSpan={8} className="py-6 text-center text-slate-400">Loading…</td></tr>
               : list.length === 0 ? <tr><td colSpan={8} className="py-6 text-center text-slate-400">No payroll for this month yet.</td></tr>
               : list.map(p => (
                <tr key={p.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                  ...
                  <td className="py-2.5 px-5 text-right"><button onClick={() => download(p)} className="sk-btn-ghost py-1.5 text-xs"><Download className="w-3.5 h-3.5" /> PDF</button></td>
                </tr>
              ))}
            </tbody>
          </table>
