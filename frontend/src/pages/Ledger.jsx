...
export default function Ledger() {
  ...
  const load = async () => {
    ...
    const r = await api.get("/ledger", { params });
    setItems(r.data);
    if (empId) {
      const b = await api.get(`/ledger/balance/${empId}`);
      setBalance(b.data);
  ...
  return (
    <div className="sk-page space-y-5" data-testid="ledger-page">
      <div>
        <h1 className="font-heading text-2xl md:text-3xl font-extrabold">Ledger / Khata</h1>
        <p className="text-sm text-slate-500 mt-1">Advances, allowances and deductions per employee</p>
      </div>
      ...
      {isAdmin && (
        <form onSubmit={submit} className="sk-card p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="font-heading font-bold">Add Entry</div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <select required value={form.employee_id} onChange={e => setForm({ ...form, employee_id: e.target.value })} className="sk-input md:col-span-2" data-testid="ledger-employee">
              <option value="">Select employee…</option>
              {employees.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}
            </select>
            <select value={form.entry_type} onChange={e => setForm({ ...form, entry_type: e.target.value })} className="sk-input" data-testid="ledger-type">
              <option value="advance">Advance</option>
              <option value="allowance">Allowance</option>
              <option value="deduction">Deduction</option>
            </select>
            <input required type="number" step="1" placeholder="Amount" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} className="sk-input" data-testid="ledger-amount" />
          ...
          <div className="mt-3 flex gap-2">
            ...
            <button disabled={busy} className="sk-btn-primary" data-testid="ledger-submit">
      ...
      <div className="sk-card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2">
          ...
          <div className="font-heading font-bold">Entries</div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 && (
                <tr><td colSpan={isAdmin ? 6 : 4} className="py-6 text-center text-slate-400">No entries</td></tr>
              )}
              {items.map(l => (
                <tr key={l.id} className="border-b border-slate-100">
                  ...
                  <td className="py-2.5 pr-4">
                    <span className={`sk-badge ${l.entry_type === "advance" ? "sk-badge-warning" : l.entry_type === "allowance" ? "sk-badge-success" : "sk-badge-danger"}`}>{l.entry_type}</span>
                  </td>
                  ...
                  {isAdmin && (
                    <td className="py-2.5 px-5 text-right">
                      <button onClick={() => del(l.id)} className="text-red-500 hover:text-red-700">
