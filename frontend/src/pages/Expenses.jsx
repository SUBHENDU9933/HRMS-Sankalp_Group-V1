...
import { listExpenses, createExpense, deleteExpense } from "@/lib/data";
...
export default function Expenses() {
  ...
  const [form, setForm] = useState({ category: "Office", amount: "", description: "", expense_date: todayISO(), paid_by: "" });

  const load = async () => { setItems(await listExpenses({ category: category || undefined, date_from: dateFrom || undefined, date_to: dateTo || undefined })); };
  ...
  const submit = async (e) => {
    e.preventDefault();
    setBusy(true);
    try { await createExpense({ ...form, amount: Number(form.amount) }); toast.success("Expense added"); setForm({ ...form, amount: "", description: "", paid_by: "" }); load(); }
    catch (e) { toast.error(e.message || "Failed"); }
    finally { setBusy(false); }
  };
  const del = async (id) => { if (!window.confirm("Delete this expense?")) return; await deleteExpense(id); toast.success("Deleted"); load(); };

  return (
    <div className="sk-page space-y-5" data-testid="expenses-page">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div><h1 className="font-heading text-2xl md:text-3xl font-extrabold">Expenses</h1>
          <p className="text-sm text-slate-500 mt-1">Track operational expenses with category & date filters</p></div>
        <div className="sk-card px-4 py-3"><div className="text-xs font-semibold text-slate-500">Filtered Total</div><div className="font-heading text-2xl font-extrabold text-[#FFA94D]">{fmtINR(total)}</div></div>
      </div>
      <form onSubmit={submit} className="sk-card p-5">
        <div className="flex items-center gap-2 mb-3"><Plus className="w-4 h-4 text-[#4DA3FF]" /><div className="font-heading font-bold">Add Expense</div></div>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className="sk-input">{CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select>
      ...
      <div className="sk-card overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-2"><Receipt className="w-4 h-4 text-[#4DA3FF]" /><div className="font-heading font-bold">All Expenses</div></div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-xs uppercase tracking-wider text-slate-500 border-b">
              <th className="py-2.5 px-5">Date</th><th className="py-2.5 pr-4">Category</th>
              <th className="py-2.5 pr-4 text-right">Amount</th><th className="py-2.5 pr-4">Paid by</th>
            ...
            <tbody>
              ...
              {items.map(e => (
                <tr key={e.id} className="border-b border-slate-100">
                  ...
                  {isAdmin && <td className="py-2.5 px-5 text-right"><button onClick={() => del(e.id)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4" /></button></td>}
                </tr>
              ))}
            </tbody>
          </table>
