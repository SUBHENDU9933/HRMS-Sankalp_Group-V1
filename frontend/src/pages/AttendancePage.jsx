...
import { Camera, MapPin, CheckCircle2, AlertCircle, Clock, Filter, Trash2, Edit3 } from "lucide-react";
...
export default function AttendancePage() {
  ...
  const loadMe = async () => {
    ...
    setMe(r.data);
    setToday(r.data.find(a => a.date === todayISO()) || null);
  };
  ...
  const onCapture = async (dataUrl, gps) => {
    ...
    setBusy(true);
    try {
      const up = await api.post("/upload", { data_url: dataUrl, folder: "attendance" });
      await api.post("/attendance", {
        date: todayISO(),
        status: "present",
        selfie_url: up.data.url,
        latitude: gps?.latitude,
        longitude: gps?.longitude,
      });
      toast.success("Attendance marked");
      await loadMe();
      if (canAdmin) await loadAll();
  ...
  const StatusIcon = ({ s }) =>
    s === "present" ? <CheckCircle2 className="w-4 h-4 text-green-500" /> :
    s === "absent" ? <AlertCircle className="w-4 h-4 text-red-500" /> :
  ...
  return (
    <div className="sk-page space-y-5" data-testid="attendance-page">
      ...
      <div className="sk-card p-5">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            {today ? (
              <div className="mt-2 flex items-center gap-2">
                <StatusIcon s={today.status} />
                <span className="font-heading text-2xl font-extrabold capitalize">{today.status.replace("_", " ")}</span>
                <span className="text-xs text-slate-500">at {fmtDateTime(today.check_in_time)}</span>
              </div>
            ) : (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setShowCapture(true)} disabled={busy} className="sk-btn-accent" data-testid="mark-attendance-button">
          ...
          <div className="mt-4 flex gap-3 items-center">
            ...
            {today.latitude != null && (
              <span className="gps-pill">
                ...
                {today.latitude.toFixed(5)}, {today.longitude.toFixed(5)}
              </span>
            )}
          </div>
        )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wider text-slate-500 border-b">
                ...
                <th className="py-2 pr-4">Selfie</th>
      ...
      {/* Admin / manager: all attendance */}
      {canAdmin && (
        <div className="sk-card p-5">
          <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            <div className="font-heading font-bold">All Attendance Records</div>
            <div className="flex flex-wrap items-center gap-2">
              <select value={empId} onChange={e => setEmpId(e.target.value)} className="sk-input w-auto" data-testid="filter-employee">
                  ...
                  <tr key={a.id} className="border-b border-slate-100">
                    ...
                    <td className="py-2 pr-4">
                      <select
                        ...
                        className={`sk-input w-auto py-1 text-xs ${a.status === "present" ? "text-green-700" : a.status === "absent" ? "text-red-700" : "text-amber-700"}`}
                      >
                    ...
                    <td className="py-2 pr-4 text-right">
                      {isAdmin && (
                        <button onClick={() => del(a.id)} className="text-red-500 hover:text-red-700" data-testid={`delete-att-${a.id}`}>
      ...
      {showCapture && (
        <SelfieCapture employeeName={user.name} onCapture={onCapture} onClose={() => setShowCapture(false)} />
      )}
    </div>
  );
}
