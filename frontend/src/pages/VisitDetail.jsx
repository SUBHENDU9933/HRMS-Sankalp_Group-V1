...
import { ArrowLeft, MapPin, Phone, FileText, Trash2, Image as ImageIcon } from "lucide-react";
...
export default function VisitDetail() {
  ...
  useEffect(() => {
    api.get(`/visits/${id}`).then(r => setV(r.data)).catch(() => nav("/visits"));
  }, [id]);
  ...
  return (
    <div className="sk-page space-y-5 max-w-4xl" data-testid="visit-detail">
      <div className="flex items-center justify-between">
        <Link to="/visits" className="text-sm text-slate-600 inline-flex items-center gap-1.5 hover:text-slate-900">
          <ArrowLeft className="w-4 h-4" /> Back to visits
        </Link>
        {isAdmin && (
          <button onClick={del} className="text-sm text-red-600 hover:text-red-700 inline-flex items-center gap-1.5" data-testid="delete-visit-button">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          <div>
            <div className="flex items-center gap-2">
              <span className={`sk-badge ${v.visit_type === "lead" ? "sk-badge-info" : "sk-badge-warning"}`}>
                {v.visit_type === "lead" ? "Lead Visit" : "Project Visit"}
              </span>
              <span className={`sk-badge ${v.status === "completed" ? "sk-badge-success" : "sk-badge-warning"}`}>
                {v.status === "completed" ? "Completed" : "Follow-up required"}
              </span>
            </div>
            <h1 className="font-heading text-2xl md:text-3xl font-extrabold mt-2">
              {v.visit_type === "lead" ? v.lead_name : v.project_name || "Untitled"}
            </h1>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            {v.latitude && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="gps-pill bg-[#4DA3FF]/10 text-[#4DA3FF] border-[#4DA3FF]/20 hover:bg-[#4DA3FF]/20"
              >
                Open in Maps
              </a>
            )}
      ...
      {v.visit_type === "lead" ? (
        <div className="sk-card p-5 space-y-4">
          ...
          <Row label="Measurement Details" value={v.measurement_details} multiline />
      ...
      {v.site_photos?.length > 0 && (
        <div className="sk-card p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {v.site_photos.map((p, i) => (
              <a key={i} href={p} target="_blank" rel="noreferrer" className="aspect-square rounded-lg overflow-hidden border">
      ...
      {v.floor_plan_url && (
        <div className="sk-card p-5">
          ...
          <a href={v.floor_plan_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-[#4DA3FF] font-medium">
            <FileText className="w-4 h-4" /> Open floor plan
          </a>
        </div>
      )}

      {v.notes && v.visit_type === "lead" && (
        <div className="sk-card p-5">
          <div className="font-heading font-bold mb-1">Notes</div>
          <div className="text-sm text-slate-700 whitespace-pre-wrap">{v.notes}</div>
        </div>
      )}
    </div>
  );
...
const Row = ({ label, value, icon: Icon, multiline }) => {
  ...
  return (
    <div className="flex gap-3 items-start">
      ...
      <div className={`text-sm text-slate-800 flex-1 ${multiline ? "whitespace-pre-wrap" : ""}`}>
        {Icon && <Icon className="w-3.5 h-3.5 inline-block mr-1 text-slate-400" />}
        {value}
      </div>
    </div>
  );
