...
export default function ApplicationDetail() {
  ...
  const doChange = async (new_status, extra = {}) => {
    setBusy(true);
    try {
      const wasReschedule = new_status === "interview_scheduled" && app?.status === "interview_scheduled";
      await changeStatus({ application_id: id, new_status, ...extra });
      toast.success(wasReschedule ? "Interview rescheduled" : `Status updated to ${STATUS_LABEL[new_status]}`);
      setPanel(null);
      load();
      if (EMAIL_ON.has(new_status)) {
        const emailKind = wasReschedule ? "interview_rescheduled" : new_status;
        const r = await sendStatusEmail(id, emailKind);
  ...
  const currentEmailKind = app.status === "interview_scheduled" && app.last_emailed_kind === "interview_rescheduled"
    ? "interview_rescheduled" : app.status;
  ...
  return (
    <div className="sk-page max-w-3xl">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div><h1 className="font-heading text-2xl md:text-3xl font-extrabold" style={{ color: ROYAL }}>{app.name}</h1><div className="text-sm text-slate-500 font-mono">{app.application_number} • {app.job_title}</div></div>
      ...
      <div className="sk-card p-5 mt-4 space-y-3">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1 text-sm flex-1">
            <div><span className="text-slate-400">Email:</span> {app.email}</div><div><span className="text-slate-400">Phone:</span> {app.phone}</div>
            <div><span className="text-slate-400">Experience:</span> {app.experience_years ?? "—"} yrs</div><div><span className="text-slate-400">Current Company:</span> {app.current_company || "—"}</div>
      ...
      {(app.interview_date || app.interviewer_name) && <div className="sk-card p-5 mt-4 space-y-1 text-sm"><div className="font-heading font-bold mb-1">Interview</div><div><span className="text-slate-400">Date/Time:</span> {app.interview_date} {app.interview_time}</div><div><span className="text-slate-400">Mode:</span> {app.interview_mode}</div><div><span className="text-slate-400">Interviewer:</span> {app.interviewer_name || "—"}</div>{app.interview_feedback && <div><span className="text-slate-400">Feedback:</span> {app.interview_feedback}</div>}{app.interview_rating && <div><span className="text-slate-400">Rating:</span> {app.interview_rating}/5</div>}</div>}
      {app.joining_date && <div className="sk-card p-5 mt-4 text-sm"><div className="font-heading font-bold mb-1">Joining</div><div><span className="text-slate-400">Joining Date:</span> {app.joining_date}</div></div>}
      <div className="sk-card p-5 mt-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {EMAIL_ON.has(app.status) && <button disabled={busy} className="sk-btn-ghost" onClick={async () => { const r = await sendStatusEmail(id, currentEmailKind, true); toast[r?.ok === false ? "error" : "success"](r?.ok === false ? "Resend failed" : "Email resent"); load(); }}><Mail className="w-4 h-4" /> Resend Email</button>}
          {WHATSAPP_ON.has(app.status) && <button disabled={busy} className="sk-btn-ghost" onClick={openWhatsApp}><MessageCircle className="w-4 h-4" /> WhatsApp Message</button>}
        </div>
        {app.last_email_error && <div className="text-xs text-red-600 flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5" /> Last email failed: {app.last_email_error}</div>}
        {panel === "interview" && <div className="border-t border-slate-100 pt-4 mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <F label="Mode *"><select className="sk-input" value={iv.interview_mode} onChange={e => setIv({ ...iv, interview_mode: e.target.value })}><option value="in_person">In Person</option><option value="phone">Phone</option><option value="video">Video</option></select></F>
          <F label="Interviewer *"><select className="sk-input" value={iv.interviewer} onChange={e => setIv({ ...iv, interviewer: e.target.value })}><option value="">Select…</option>{interviewers.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select></F>
          <div className="md:col-span-2 flex justify-end gap-2"><button className="sk-btn-ghost" onClick={() => setPanel(null)}>Cancel</button><button disabled={busy || !iv.interview_date || !iv.interview_time || !iv.interviewer} className="sk-btn-primary" style={{ background: ROYAL, borderColor: ROYAL }} onClick={() => doChange("interview_scheduled", iv)}>{busy && <Loader2 className="w-4 h-4 animate-spin" />} Confirm {canReschedule ? "Reschedule" : "Schedule"}</button></div>
        </div>}
        {panel === "outcome" && <div className="border-t border-slate-100 pt-4 mt-2 space-y-3"><F label="Interview Feedback"><textarea rows={3} className="sk-input" value={outcome.interview_feedback} onChange={e => setOutcome({ ...outcome, interview_feedback: e.target.value })} /></F><F label="Rating (1–5)"><input type="number" min="1" max="5" className="sk-input" value={outcome.interview_rating} onChange={e => setOutcome({ ...outcome, interview_rating: e.target.value })} /></F><div className="flex justify-end gap-2"><button className="sk-btn-ghost" onClick={() => setPanel(null)}>Cancel</button><button disabled={busy} className="sk-btn-primary" style={{ background: ROYAL, borderColor: ROYAL }} onClick={() => doChange("interviewed", { interview_feedback: outcome.interview_feedback || null, interview_rating: outcome.interview_rating ? Number(outcome.interview_rating) : null })}>{busy && <Loader2 className="w-4 h-4 animate-spin" />} Save Outcome</button></div></div>}
        {panel === "joining" && <div className="border-t border-slate-100 pt-4 mt-2 space-y-3"><F label="Joining Date *"><input type="date" required className="sk-input" value={joiningDate} onChange={e => setJoiningDate(e.target.value)} /></F><div className="flex justify-end gap-2"><button className="sk-btn-ghost" onClick={() => { setPanel(null); setJoiningDate(""); }}>Cancel</button><button disabled={busy || !joiningDate} className="sk-btn-primary" style={{ background: ROYAL, borderColor: ROYAL }} onClick={() => doChange("joining", { joining_date: joiningDate })}>{busy && <Loader2 className="w-4 h-4 animate-spin" />} Confirm Joining</button></div></div>}
        {panel === "reassign" && <div className="border-t border-slate-100 pt-4 mt-2 space-y-3"><F label="Reassign To Position *"><select className="sk-input" value={reassignTo} onChange={e => setReassignTo(e.target.value)}><option value="">Select a position…</option>{openings.filter(o => o.id !== app.job_opening_id).map(o => <option key={o.id} value={o.id}>{o.title}</option>)}</select></F><F label="Note (optional)"><textarea rows={2} className="sk-input" value={reasonNote} onChange={e => setReasonNote(e.target.value)} /></F><div className="text-xs text-slate-500">This resets the application to "New" on the selected position — the candidate's original details, CV, and full history stay attached to this same application.</div><div className="flex justify-end gap-2"><button className="sk-btn-ghost" onClick={() => { setPanel(null); setReasonNote(""); setReassignTo(""); }}>Cancel</button><button disabled={busy || !reassignTo} className="sk-btn-primary" style={{ background: ROYAL, borderColor: ROYAL }} onClick={doReassign}>{busy && <Loader2 className="w-4 h-4 animate-spin" />} Confirm Reassign</button></div></div>}
        {(panel === "reject" || panel === "hold") && <div className="border-t border-slate-100 pt-4 mt-2 space-y-3"><F label="Note (optional)"><textarea rows={2} className="sk-input" value={reasonNote} onChange={e => setReasonNote(e.target.value)} /></F><div className="flex justify-end gap-2"><button className="sk-btn-ghost" onClick={() => { setPanel(null); setReasonNote(""); }}>Cancel</button><button disabled={busy} className="sk-btn-primary" style={{ background: ROYAL, borderColor: ROYAL }} onClick={() => doChange(panel === "reject" ? "rejected" : "on_hold", { note: reasonNote || null })}>{busy && <Loader2 className="w-4 h-4 animate-spin" />} Confirm {panel === "reject" ? "Reject" : "On Hold"}</button></div></div>}
      </div>
