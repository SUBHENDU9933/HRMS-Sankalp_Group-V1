...
import {
  Users, ClipboardCheck, MapPinned, Wallet, Receipt, ChevronRight,
  CheckCircle2, AlertCircle, Clock, Sparkles
} from "lucide-react";

const Card = ({ icon: Icon, label, value, hint, color = "blue", testId }) => {
  const colors = {
    blue: "bg-blue-50 text-[#4DA3FF]",
    orange: "bg-orange-50 text-[#FFA94D]",
  ...
  return (
    <div className="sk-card p-5 hover:shadow-md transition-shadow" data-testid={testId}>
      <div className="flex items-start justify-between">
        ...
        <div className={`w-10 h-10 rounded-xl ${colors[color]} grid place-items-center`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 sk-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/visits" className="text-xs text-[#4DA3FF] font-semibold hover:underline flex items-center gap-1">
              View all <ChevronRight className="w-3 h-3" />
            </Link>
          </div>
          {data.recent_visits.length === 0 ? (
            <EmptyState icon={MapPinned} label="No visits yet" />
          ) : (
              ...
              {data.recent_visits.map(v => (
                <li key={v.id}>
                  <Link to={`/visits/${v.id}`} className="flex items-center gap-3 py-3 hover:bg-slate-50 -mx-2 px-2 rounded-lg transition">
                    {v.selfie_url ? (
                      <img src={v.selfie_url} alt="" className="w-11 h-11 rounded-lg object-cover" />
                    ) : (
                      <div className="w-11 h-11 rounded-lg bg-slate-100 grid place-items-center">
        ...
        <div className="sk-card p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-heading text-lg font-bold">Top Visitors</h2>
            <span className="text-xs text-slate-400">This month</span>
          </div>
          {data.top_employees_by_visits.length === 0 ? (
            <EmptyState icon={Sparkles} label="No data this month" />
          ) : (
            <ul className="space-y-3">
              {data.top_employees_by_visits.map((e, i) => (
                <li key={e.employee_id} className="flex items-center gap-3">
                  <div className={`w-7 h-7 rounded-full grid place-items-center text-xs font-bold ${
                    i === 0 ? "bg-[#FFA94D] text-white" : "bg-slate-100 text-slate-600"
                  }`}>
                    {i + 1}
                  </div>
                  <div className="flex-1 text-sm font-medium text-slate-800 truncate">{e.name}</div>
                  <div className="text-xs font-bold text-[#4DA3FF]">{e.count}</div>
                </li>
              ))}
            </ul>
          )}
          <div className="mt-5 pt-4 border-t border-slate-100">
            <div className="text-xs text-slate-500 mb-1">Expenses (this month)</div>
            <div className="font-heading text-2xl font-extrabold text-slate-900">{fmtINR(data.expenses_this_month)}</div>
          </div>
        </div>
      </div>
    </>
            ...
            <ul className="divide-y divide-slate-100">
              {data.recent_visits.map(v => (
                <li key={v.id}>
                  <Link to={`/visits/${v.id}`} className="flex items-center gap-3 py-3 hover:bg-slate-50 -mx-2 px-2 rounded-lg transition">
                    {v.selfie_url ? (
                      <img src={v.selfie_url} alt="" className="w-10 h-10 rounded-lg object-cover" />
                    ) : (
