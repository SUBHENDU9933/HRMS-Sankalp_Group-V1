  ...
  return (
    <div className="sk-page space-y-5" data-testid="employees-page">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        ...
        {isAdmin && (
          <Link to="/employees/new" className="sk-btn-primary" data-testid="add-employee-button">
      ...
      {loading ? (
        ...
        <div className="sk-card p-10 text-center">
          <UserCircle className="w-10 h-10 mx-auto text-slate-300 mb-2" />
          <div className="font-semibold">No employees</div>
          <div className="text-sm text-slate-500">Add your first team member to get started.</div>
        </div>
      ) : (
            ...
            <div key={e.id} className="sk-card p-4" data-testid={`employee-card-${e.id}`}>
              <div className="flex items-start gap-3">
                {e.photo_url ? (
                  ...
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#4DA3FF] to-[#FFA94D] grid place-items-center text-white font-bold">
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-100">
                <div className="text-xs text-slate-500">
                  {e.salary_type === "monthly" ? `₹${e.monthly_salary || 0}/mo` : `₹${e.daily_rate || 0}/day`}
                </div>
                <div className="flex gap-1">
                  <Link to={`/employees/${e.id}`} className="p-1.5 rounded text-slate-500 hover:bg-slate-100 hover:text-[#4DA3FF]" data-testid={`edit-${e.id}`}>
                    <Edit3 className="w-4 h-4" />
                  </Link>
                  {isAdmin && (
                    <button onClick={() => del(e.id)} className="p-1.5 rounded text-slate-500 hover:bg-red-50 hover:text-red-500" data-testid={`del-${e.id}`}>
