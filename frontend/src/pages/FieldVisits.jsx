...
import { Plus, MapPinned, Search, Filter, ChevronRight } from "lucide-react";
...
export default function FieldVisits() {
  ...
  const load = async () => {
    setLoading(true);
    const params = {};
    if (type) params.visit_type = type;
  ...
  return (
    <div className="sk-page space-y-5" data-testid="visits-page">
      <div className="flex items-end justify-between gap-3 flex-wrap">
        ...
        <Link to="/visits/new" className="sk-btn-accent" data-testid="add-visit-button">
      ...
      <div className="sk-card p-3 md:p-4 flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Search lead / project / employee"
            className="sk-input pl-9"
            data-testid="visit-search"
          />
        </div>
        <select value={type} onChange={e => setType(e.target.value)} className="sk-input w-auto" data-testid="visit-type-filter">
          <option value="">All Types</option>
          <option value="lead">Lead Visit</option>
          <option value="project">Project Visit</option>
        </select>
        <select value={status} onChange={e => setStatus(e.target.value)} className="sk-input w-auto" data-testid="visit-status-filter">
      ...
      {loading ? (
        ...
        <div className="sk-card p-10 text-center">
          <div className="w-14 h-14 mx-auto rounded-full bg-slate-100 grid place-items-center mb-3">
          ...
          <Link to="/visits/new" className="sk-btn-primary mt-4 inline-flex">
            <Plus className="w-4 h-4" /> Add Visit
          </Link>
        </div>
      ) : (
          ...
          {filtered.map(v => (
            <Link key={v.id} to={`/visits/${v.id}`} className="sk-card overflow-hidden hover:shadow-md transition group" data-testid={`visit-card-${v.id}`}>
              <div className="aspect-video bg-slate-100 relative">
                {v.selfie_url ? (
                  <img src={v.selfie_url} alt="" className="w-full h-full object-cover" />
                ) : (
                ...
                <div className="absolute top-2 left-2 flex gap-2">
                  <span className={`sk-badge ${v.visit_type === "lead" ? "sk-badge-info" : "sk-badge-warning"}`}>
                    {v.visit_type === "lead" ? "Lead" : "Project"}
                  </span>
                  <span className={`sk-badge ${v.status === "completed" ? "sk-badge-success" : "sk-badge-warning"}`}>
                    {v.status === "completed" ? "Done" : "Follow-up"}
                  </span>
                </div>
              </div>
              <div className="p-4">
                <div className="font-heading font-bold text-slate-900 truncate">
                  {v.visit_type === "lead" ? (v.lead_name || "Lead Visit") : (v.project_name || "Project Visit")}
                </div>
                <div className="text-xs text-slate-500 mt-0.5 truncate">
                  {v.visit_type === "lead" ? v.lead_location : v.project_location || "—"}
                </div>
